...
public class DrugsController {
	 ...
	 * public Drugs saveDrugsDetails(@RequestPart("drugs") Drugs
	 * drugs, @RequestPart("imageFile") MultipartFile[] file) throws
	 * CustomException, Exception { // return service.saveDrugsDetails(drugs); try {
	 * return service.saveDrugsDetails(drugs,file); }catch(Exception e){
	...
	@GetMapping("/{id}")
	public Optional<Drugs> getDrugsById(@PathVariable("id") int drugId) throws CustomException, Exception{
		return service.getDrugsByID(drugId);
	}
	...
	@DeleteMapping("/{id}")
	public String deleteDrugs(@PathVariable("id") int drugId) {

		return service.deleteDrugs(drugId);
